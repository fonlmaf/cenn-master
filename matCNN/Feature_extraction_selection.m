function[ features,names ] = Feature_extraction_selection(dir_gray,dir_binary)  
 ...
 for i = 1 : num_bmps  
   ...
   [height,width]=size(img_gray);
    ...
    %h=imhist(img_gray);
    %h(1)=0;
    feature(1)=std2(imread(strcat('.\CENNFIG\all-mias\ROI\',bmps(i).name)));
    %GLCM_Contrast
    glcm = graycomatrix(img_gray);
    stats = graycoprops(glcm);
    feature(2)=stats.Contrast;
    %%%regionprops
    stats = (regionprops(img_bin,'ConvexArea','Solidity'));
    %convex area
    feature(3)=stats.ConvexArea/(height*width);
    
    %spiculated
    feature(4) = curvature_angle(img_bin);
    
    %solidity
    feature(5) = stats.Solidity;
