...
if ~isfield(mCNN,'TimeComplexity')    mCNN.TimeComplexity=0; end
if ~isfield(mCNN,'SimulTime')    mCNN.SimulTime=0; end
...
% check for the num of used LAMs in mCNN structure
count=0;
field_dontcare={'STATE','OUTPUT','INPUT1','INPUT2','MASK','BIAS'};
mCNNfields=fieldnames(mCNN);
for i=1:length(mCNNfields) %throw all fields
    if size(mCNN.(mCNNfields{i}))>=(size(mCNN.STATE)-1) % those of size as big as STATE
        to_count=1;        
        for j=1:length(field_dontcare) %check wether is in the do not care list
            if length(mCNNfields{i})==length(field_dontcare{j})
                if mCNNfields{i}==field_dontcare{j}
                    to_count=0;
                    break;
                end
            end
...
if count>mCNN.MaxUsedLAM
    mCNN.MaxUsedLAM=count;
end

mCNN.SimulTime=mCNN.SimulTime+Rt;
mCNN.TimeComplexity=mCNN.TimeComplexity+mCNN.IterNum*mCNN.TimeStep;
