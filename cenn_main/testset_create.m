...
TempA = [4.4753 0.1577 4.4904;
              -0.0703  -8.0000 -0.0703; 
              4.4904 0.1577 4.4753];
 TempB = [-2.6069 2.8645 7.9711;
              -1.3240 -1.3260 -1.3240; 
               7.9711 2.8645 -2.6069];
%  bias=-5.7713;
...
bmps = dir(strcat('..\ROIextraction\','*.bmp'));
num = length( bmps ); 
%num = 2;
diffSum1=zeros(num,1);

for iter=1:num
    png_file=fullfile( '..\ROIextraction\' , bmps(iter).name );
%     [ path , name , ext ] = fileparts( png_file ) ;  

  % 第二步，生成新的文件名   
    real=im2double(imread(png_file)); 
    test = lbmp2cnn(fullfile('..\Imgextraction\' , bmps(iter).name));
